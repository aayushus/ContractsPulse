...
## 2024-08-01 - Exposing Invisible Keyboard Shortcuts
**Learning:** Invisible keyboard shortcuts (such as pressing 'Enter' to submit a chat or form) are undiscoverable to users unless explicitly communicated. By adding a `title` attribute to the associated action button (e.g., `title="Send (Enter)"`), we expose the shortcut visually to mouse users who rely on tooltips, reducing user friction and improving overall usability.
**Action:** Always expose keyboard shortcut hints via a `title` attribute (or a tooltip) on the associated action button to improve discoverability.
